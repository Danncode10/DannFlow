        ...
        <div className="flex items-start justify-center gap-0 mt-14 flex-wrap">
          {flowSteps.map((step, i) => (
            <div key={i} className="flex flex-col items-center max-w-[140px]">
              <div style={{ width: '48px', height: '48px', borderRadius: '50%', border: `2px solid ${step.isGold ? 'rgba(201,168,76,0.6)' : 'rgba(255,255,255,0.3)'}`, backgroundColor: step.isGold ? 'rgba(201,168,76,0.15)' : 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', marginBottom: '1rem', transition: 'all 0.2s', cursor: 'pointer' }} onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.2)'; e.currentTarget.style.borderColor = '#C9A84C'; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = step.isGold ? 'rgba(201,168,76,0.15)' : 'rgba(255,255,255,0.1)'; e.currentTarget.style.borderColor = step.isGold ? 'rgba(201,168,76,0.6)' : 'rgba(255,255,255,0.3)'; }}>{step.emoji}</div>
              <div style={{ fontSize: '0.75rem', fontWeight: 600, textAlign: 'center', color: 'rgba(255,255,255,0.8)', lineHeight: 1.4 }}>{step.label}</div>
              {i < flowSteps.length - 1 && <div style={{ marginTop: '24px', color: 'rgba(255,255,255,0.3)', fontSize: '1.2rem', padding: '0 0.25rem' }}>↓</div>}
            </div>
          ))}
        </div>
      </div>
