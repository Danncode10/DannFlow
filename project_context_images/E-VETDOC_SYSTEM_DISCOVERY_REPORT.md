...
4. The clinic creates an invoice; the owner views it, pays it, and receives a receipt and notifications.
...
| --- | --- | --- |
| **Pet owner / client** | Register and sign in; view their own pets and medical history; request/view appointments; view invoices; make payments; print appointment details. | Can access only their own profile, pets, appointments, records, invoices, and receipts. |
...
| **System / integrations** | Send notifications, deliver invoices and receipts, process payment confirmations, and generate documents. | Uses server-side credentials only; must not bypass user authorization. |
...
- Record payment status and payments accurately.
- Deliver digital invoices and receipts by email and/or the application.
- Support an online payment gateway or mobile-payment provider; payment confirmation must be verified server-side through provider webhooks.
...
| **Billing** | Invoice headers/lines, adjustments, payments, payment-provider references, receipts, and financial reporting. |
...
- Keep payment-provider transaction IDs and webhook event IDs for reconciliation and idempotency.
...
| Billing/payment processing | Not implemented | Add invoice/payment tables, a chosen payment provider, secure webhook handling, reconciliation, and receipts. |
...
4. **Finance:** implement invoices, line items, payments, receipt generation, and payment-provider integration.
...
6. **Hardening:** test role/clinic isolation, audit trails, backups, error handling, accessibility, and payment-webhook reliability.
...
- The images describe the desired functions but do not define essential product rules such as clinic multi-tenancy, appointment duration, cancellation policy, diagnostic vocabulary, prescription-signing policy, payment provider, or local compliance requirements. Those decisions should be confirmed before schema work.
