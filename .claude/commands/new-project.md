...
1. Read `README.md`, `PROJECT_CONTEXT.md`, `src/lib/config.ts`, `.env.example`, `package.json`, and `dannflow.json`.
...
7. After the repository and Supabase verification succeed, write non-secret initialization metadata to `dannflow.json`:
...
Preserve unrelated DannFlow metadata. Never write keys, database URLs, passwords, or tokens.
8. Hand off to `/masterplan-init`. Explain that the user must first create a non-draft GitHub Project in Kanban/Board layout with `Backlog`, `Ready`, `In progress`, and `Done` statuses; `/masterplan-init` will detect and link it.
